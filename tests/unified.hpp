...
void testUnified(int N, int startingK, int maxK, float geometricFactor, int passes) {
    cout << "# Tango Unified (" << passes << ")" << endl;
    ...
    cout << "# MultiSplay Unified (" << passes << ")" << endl;
    for (int K = startingK; K <= maxK; K *= geometricFactor) {
        std::vector<int> v(N);
        std::iota (std::begin(v), std::end(v), 0);
        std::random_device rd;
    ...
    cout << "# Splay Unified (" << passes << ")" << endl;
        for (int K = startingK; K <= maxK; K *= geometricFactor) {
        std::vector<int> v(N);
        std::iota (std::begin(v), std::end(v), 0);
        std::random_device rd;
