...
void testEntropy(int N, float startingK, float maxK, float geometricFactor, int passes) {

    cout << "# Tango Entropy (" << passes << ")" << endl;
    ...
    cout << "# MultiSplay Entropy (" << passes << ")" << endl;
    for (float K = startingK; K <= maxK; K = (1 - (1 - K)/geometricFactor)) {
        std::default_random_engine generator;
        zipfian_int_distribution<int> distribution(1, N, K);
        std::vector<int> v(N);
    ...
    cout << "# Splay Entropy (" << passes << ")" << endl;
    for (float K = startingK; K <= maxK; K = (1 - (1 - K)/geometricFactor)) {
        std::default_random_engine generator;
        zipfian_int_distribution<int> distribution(1, N, K);
        std::vector<int> v(N);
