...
class TangoTree : public BST<K, V, TangoNodeInfo> {
   private:
    ...
    void left_rotate(TangoNode<K, V> *node) {
        ...
        operations += 2; // one operation to go down left child and one operation to rotate
        ...
        while (parent != nullptr && parent->info.red) {
            ...
            if (parent == grandparent->left) {
                ...
                if (uncle != nullptr && uncle->info.red) {  // case 1: UNCLE IS RED
                    ...
                    // going to grandparent is free since we are at parent
    ...
    TangoNode<K, V> *fixupMergeTango(TangoNode<K, V> *node) {
        ...
        while (node->parent && !node->info.marked && ++operations) {
            node = node->parent;
        }
        if (node->info.red) {
            node->info.red = false;
    ...
    TangoNode<K, V> *concatenate_left(TangoNode<K, V> *left, TangoNode<K, V> *pivot,
        ...
        while (current->info.black_height > right->info.black_height && (operations += 3)) {
            current = current->right;
        }

        // this can be done by single rotations
    ...
    TangoNode<K, V> *concatenate_right(TangoNode<K, V> *left, TangoNode<K, V> *pivot,
        ...
        while (current->info.black_height > left->info.black_height && (operations += 3)) {
    ...
    int findDepth(TangoNode<K, V> *root, TangoNode<K, V> *node) {
        int result = 0;
        while (node != root) {
            node = node->parent;
            result++;
    ...
    TangoNode<K, V> *cut(TangoNode<K, V> *root, int depth) {
        ...
        if (L) operations += findDepth(root, L);
        else operations += findDepth(root, l);
        auto r = maxWithDepth(root, depth);
        TangoNode<K, V> *R = succ(r);
        if (R) operations += findDepth(root, R);
        else operations += findDepth(root, r);
        if (L && R) {
            split(root, L);
            split(L->right, R);
            auto B = R->left;
    ...
    TangoNode<K, V> *join(TangoNode<K, V> *root, TangoNode<K, V> *other) {
        ...
        if (LR->right == other) {
            ...
            R = succ(L);
            if (R) operations += findDepth(root, R);
            else operations += findDepth(root, LR);
        } else {
            R = LR;
            L = pred(R);
            if (L) operations += findDepth(root, L);
