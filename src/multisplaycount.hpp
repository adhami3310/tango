...
private:
    ...
    int numChildren(MultiSplayNode<K, V>* node) {
    ...
    // Doesn't count operations, should be counted by caller
    ...
    // Counts rotation, but doesn't count depth operations; those should be counted by caller.
    ...
    bool splayStep(MultiSplayNode<K, V>* node, MultiSplayNode<K, V>* root) {
        ...
        if (par->parent == root || par->info.marked) {
            operations += 2*(numChildren(par)+numChildren(node)-1); // getting depth info
            operations += 2; // go to parent, come back to node (update depths on the way)
            rotate(node);
            return false;
        }

        // for checking is_left_child, must go to grandparent
        if (node->is_left_child() == par->is_left_child()) {
            operations += 2*(numChildren(node) + numChildren(par) + numChildren(par->parent) - 2); // get depth info
            operations += 3; // go to grandparent, come back to  (update depths on the way)
            
            // zig-zig and zag-zag
            rotate(par);
            rotate(node);
            operations ++; // 1 follow par->node
       } else {
            operations += 2*(numChildren(node) + numChildren(par) + numChildren(par->parent) - 2); // get depth info
            operations += 4; // go to grandparent, come back to node (update depths)
    ...
    void switchLeftToRight(MultiSplayNode<K, V>* node) {
        ...
        // backtrack from l to root is paid for by splaying
        if (l) {
            splayUpto(l, node);
            if (l->right) {
        ...
        operations += 2; // go to left, back to node

        auto r = minWithDepthLessThan(node->right, node->info.reference_depth);
        // backtrack from r to root is paid for by splaying
        if (r) {
            splayUpto(r, node);
            if (r->left) {
        ...
        operations += 2; // go to right, back to node

        // depth info already fetched while splaying l, r
        updateMinDepth(node);
    }
...
public:
    ...
    MultiSplayNode<K, V>* find(K key) {
        ...
        while (true) {
            ...
            operations++; // follow left/right pointer
        }
